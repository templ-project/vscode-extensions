...
class VersionManager {
  ...
  storeCurrentVersions() {
    ...
    if (fs.existsSync(PACKAGES_DIR)) {
      const ides = fs.readdirSync(PACKAGES_DIR).filter(item => 
        fs.statSync(path.join(PACKAGES_DIR, item)).isDirectory()
      );
      
      for (const ide of ides) {
        const ideDir = path.join(PACKAGES_DIR, ide);
        const languages = fs.readdirSync(ideDir).filter(item => 
          fs.statSync(path.join(ideDir, item)).isDirectory()
        );
