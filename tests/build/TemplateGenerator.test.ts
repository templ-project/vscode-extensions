...
{{/each}}
    ...
    test('throws BuildError when template file not found', async () => {
      ...
      await expect(generator.render('nonexistent.handlebars', {})).rejects.toThrow(BuildError);

      await expect(generator.render('nonexistent.handlebars', {})).rejects.toThrow(
        'Failed to load template: nonexistent.handlebars',
      );
    });
    ...
    test('throws BuildError on invalid template syntax', async () => {
      ...
      await expect(generator.render('invalid.handlebars', {})).rejects.toThrow(BuildError);
    });
    ...
    test('throws BuildError when required context is missing in strict mode', async () => {
      ...
      await expect(generator.render('strict.handlebars', {})).rejects.toThrow(BuildError);
    });
  });
  ...
  describe('renderToFile()', () => {
    test('renders template and writes to file', async () => {
      ...
      await generator.renderToFile('output.handlebars', { content: 'test data' }, outputPath);
    ...
    test('creates output file with correct content', async () => {
      ...
      await writeFile(templatePath, '{"name": "{{name}}", "version": "{{version}}"}');

      const outputPath = join(tempDir, 'package.json');
      const generator = new TemplateGenerator(logger, templatesDir);

      await generator.renderToFile('json.handlebars', { name: 'my-pkg', version: '1.0.0' }, outputPath);
    ...
    test('throws BuildError when output directory does not exist', async () => {
      ...
      await expect(generator.renderToFile('test.handlebars', {}, outputPath)).rejects.toThrow(BuildError);
    });
    ...
    test('overwrites existing file', async () => {
      ...
      const generator = new TemplateGenerator(logger, templatesDir);
      await generator.renderToFile('overwrite.handlebars', { value: 'replaced' }, outputPath);
  ...
  describe('Handlebars Helpers', () => {
    ...
    test('getPublisher helper extracts publisher from extension ID', async () => {
      ...
      await writeFile(templatePath, 'Publisher: {{getPublisher extensionId}}');
  ...
  describe('Edge Cases', () => {
    ...
    test('handles large template context', async () => {
      ...
      await writeFile(templatePath, '{{#each items}}{{name}},{{/each}}');
