 ...
 * ```
 */
export async function readExistingVersion(packagePath: string, logger: Logger): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      logger.error({ err: parseError, packagePath }, 'Failed to parse package.json as valid JSON');

      throw new BuildError(`Invalid JSON in package.json: ${packagePath}`, {
        packagePath,
    ...
    if (typeof packageJson !== 'object' || packageJson === null || Array.isArray(packageJson)) {
      logger.error({ packagePath, packageJson }, 'package.json is not a valid object');

      throw new BuildError(`package.json is not a valid object: ${packagePath}`, {
        packagePath,
    ...
    if (typeof pkg.version === 'string' && pkg.version.length > 0) {
      logger.debug({ packagePath, version: pkg.version }, 'Existing version found');
      return pkg.version;
    }

    // Version field missing or invalid - return default
    logger.debug({ packagePath, versionField: pkg.version }, 'No valid version field found, using default');
    ...
    if (error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT') {
      logger.debug({ packagePath }, 'package.json not found, using default version');
      return DEFAULT_VERSION;
    }

    // Other file system errors
    logger.error({ err: error, packagePath }, 'Failed to read existing package.json');

    throw new BuildError(`Failed to read package.json: ${packagePath}`, {
      packagePath,
