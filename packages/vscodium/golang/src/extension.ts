...
export function activate(context: vscode.ExtensionContext) {
    console.log('[templ-project-1753306959749] Golang Extension Pack extension is now active!');

    // Apply settings when extension activates
    applySettings();

    // Register commands
    const applySettingsCommand = vscode.commands.registerCommand('templ-project.golang-vscodium.applySettings', () => {
        applySettings();
        vscode.window.showInformationMessage('[templ-project-1753306959749] Golang Extension Pack settings applied!');
    });

    const resetSettingsCommand = vscode.commands.registerCommand('templ-project.golang-vscodium.resetSettings', () => {
        resetSettings();
        vscode.window.showInformationMessage('[templ-project-1753306959749] Golang Extension Pack settings reset!');
    });
...
export function deactivate() {
    console.log('[templ-project-1753306959749] Golang Extension Pack extension is now deactivated!');
}
