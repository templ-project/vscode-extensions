...
export function activate(context: vscode.ExtensionContext) {
  console.log('Templ Project Javascript Extension Pack extension is now active!');
  ...
  const applySettingsCommand = vscode.commands.registerCommand(
    'templ-project.javascript-vscodium.applySettings',
    () => {
      applySettings();
      vscode.window.showInformationMessage('Templ Project Javascript Extension Pack settings applied!');
    },
  );

  const resetSettingsCommand = vscode.commands.registerCommand(
    'templ-project.javascript-vscodium.resetSettings',
    () => {
      resetSettings();
      vscode.window.showInformationMessage('Templ Project Javascript Extension Pack settings reset!');
    },
  );
...
function applySettings() {
  ...
  config.update('eslint.validate', ['javascript', 'javascriptreact'], vscode.ConfigurationTarget.Global);
  config.update('eslint.format.enable', false, vscode.ConfigurationTarget.Global);
  config.update('eslint.codeActionsOnSave.rules', ['*'], vscode.ConfigurationTarget.Global);
  config.update(
    '[javascript]',
    {
...
export function deactivate() {
  console.log('Templ Project Javascript Extension Pack extension is now deactivated!');
}
