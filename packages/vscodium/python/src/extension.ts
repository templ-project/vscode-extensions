...
export function activate(context: vscode.ExtensionContext) {
    console.log('[templ-project-1753303607150] Python Extension Pack extension is now active!');
    ...
    const applySettingsCommand = vscode.commands.registerCommand('templ-project.python-vscodium.applySettings', () => {
        applySettings();
        vscode.window.showInformationMessage('[templ-project-1753303607150] Python Extension Pack settings applied!');
    });

    const resetSettingsCommand = vscode.commands.registerCommand('templ-project.python-vscodium.resetSettings', () => {
        resetSettings();
        vscode.window.showInformationMessage('[templ-project-1753303607150] Python Extension Pack settings reset!');
    });
...
function applySettings() {
  ...
  "editor.insertSpaces": true
    ...
    config.update('python.defaultInterpreterPath', 'python', vscode.ConfigurationTarget.Global);
...
export function deactivate() {
    console.log('[templ-project-1753303607150] Python Extension Pack extension is now deactivated!');
}
