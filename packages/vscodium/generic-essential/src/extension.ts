...
export function activate(context: vscode.ExtensionContext) {
    console.log('[templ-project-1753302005599] Generic Essential Extension Pack extension is now active!');

    // Apply settings when extension activates
    applySettings();

    // Register commands
    const applySettingsCommand = vscode.commands.registerCommand('templ-project.generic-essential-vscodium.applySettings', () => {
        applySettings();
        vscode.window.showInformationMessage('[templ-project-1753302005599] Generic Essential Extension Pack settings applied!');
    });

    const resetSettingsCommand = vscode.commands.registerCommand('templ-project.generic-essential-vscodium.resetSettings', () => {
        resetSettings();
        vscode.window.showInformationMessage('[templ-project-1753302005599] Generic Essential Extension Pack settings reset!');
    });
...
export function deactivate() {
    console.log('[templ-project-1753302005599] Generic Essential Extension Pack extension is now deactivated!');
}
