...
export function activate(context: vscode.ExtensionContext) {
  console.log('Templ Project Generic Essential Extension Pack extension is now active!');

  // Apply settings when extension activates
  applySettings();

  // Register commands
  const applySettingsCommand = vscode.commands.registerCommand(
    'templ-project.generic-essential-vscodium.applySettings',
    () => {
      applySettings();
      vscode.window.showInformationMessage('Templ Project Generic Essential Extension Pack settings applied!');
    },
  );

  const resetSettingsCommand = vscode.commands.registerCommand(
    'templ-project.generic-essential-vscodium.resetSettings',
    () => {
      resetSettings();
      vscode.window.showInformationMessage('Templ Project Generic Essential Extension Pack settings reset!');
    },
  );
...
function applySettings() {
  ...
  config.update('editor.rulers', [80, 120], vscode.ConfigurationTarget.Global);
  config.update('scm.defaultViewMode', 'tree', vscode.ConfigurationTarget.Global);
  config.update('errorLens.enabledDiagnosticLevels', ['error', 'warning', 'info'], vscode.ConfigurationTarget.Global);
  ...
  config.update('telemetry.enableTelemetry', false, vscode.ConfigurationTarget.Global);
  config.update('update.mode', 'manual', vscode.ConfigurationTarget.Global);
}
...
function resetSettings() {
  ...
  config.update('telemetry.enableTelemetry', undefined, vscode.ConfigurationTarget.Global);
  config.update('update.mode', undefined, vscode.ConfigurationTarget.Global);
}
...
export function deactivate() {
  console.log('Templ Project Generic Essential Extension Pack extension is now deactivated!');
}
