...
export function activate(context: vscode.ExtensionContext) {
  ...
  const applySettingsCommand = vscode.commands.registerCommand(
    'templ-project.typescript-vscodium.applySettings',
    () => {
      applySettings();
  ...
  const resetSettingsCommand = vscode.commands.registerCommand(
    'templ-project.typescript-vscodium.resetSettings',
    () => {
      resetSettings();
...
function applySettings() {
  ...
  config.update('eslint.validate', ['typescript', 'typescriptreact'], vscode.ConfigurationTarget.Global);
