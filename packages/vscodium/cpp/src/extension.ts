...
export function activate(context: vscode.ExtensionContext) {
    console.log('Templ Project Cpp Extension Pack extension is now active!');

    // Apply settings when extension activates
    applySettings();

    // Register commands
    const applySettingsCommand = vscode.commands.registerCommand('templ-project.cpp-vscodium.applySettings', () => {
        applySettings();
        vscode.window.showInformationMessage('Templ Project Cpp Extension Pack settings applied!');
    });

    const resetSettingsCommand = vscode.commands.registerCommand('templ-project.cpp-vscodium.resetSettings', () => {
        resetSettings();
        vscode.window.showInformationMessage('Templ Project Cpp Extension Pack settings reset!');
    });
...
function applySettings() {
    ...
    config.update('clangd.arguments', [
...
export function deactivate() {
    console.log('Templ Project Cpp Extension Pack extension is now deactivated!');
}
